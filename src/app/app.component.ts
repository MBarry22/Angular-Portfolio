...
export class AppComponent {
  ...
  shouldHideProject(project: any): boolean {
    if (this.categoryFilter && project.category !== this.categoryFilter) {
      return true;
    }
    if (this.tagFilter && (!project.tags || !project.tags.includes(this.tagFilter))) {
      return true;
    }
    return false;
  }
  ...
  filterProjects(projects: Project[]) {
    ...
    if (this.categoryFilter) {
      filteredProjects = filteredProjects.filter(
        (project) => project.category_id === this.categoryFilter?.id
      );

      console.log(filteredProjects)
    }
    // Tag filter (Not working) (Will fix wednesday)
    if (this.tagFilter) {
      filteredProjects = filteredProjects.filter(
        (project) => project.tags === this.tagFilter?.id
      );
